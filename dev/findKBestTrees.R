...
findKBestTrees <- function(graph, k = Inf, searchMode = "max") {
  library(igraph)
  library(optrees)
  ...
  ## partition function
  partition.MST <- function(P, MST, graph, weight) {
    ...
    for(i in 1:length(E(MST))){
      if (!E(MST)$status[i] %in% c("included", "excluded")) {
        P1$excluded <- rbind(P1$excluded, get.edgelist(MST)[i,]) # dürfte eigentlich eine leere Anweisung sein da ja aus dem MST keine excluded edges hinzugefügt werden können
        P2$included <- rbind(P2$included, get.edgelist(MST)[i,])
        ## das hier ist einfach parallisierbar: gibt eine Liste mit length(E(MST) einträgen auf die TRUE returned wird, dann über lapply bzw. foreach)
        tempMST <- calculate.MST(P1, graph = graph, weight = weight)
        if (!is.null(tempMST)) {
          MST.list.internal[[length(MST.list.internal) + 1]] <- tempMST
          partition.list.internal[[length(partition.list.internal) + 1]] <- list(included = matrix(P1$included, ncol = 2, byrow = FALSE),
                                                                                 excluded = matrix(P1$excluded, ncol = 2, byrow = FALSE))
        }
        P1 <- P2
      }
    }
  ...
  while(j < k + 1) {
    ...
    temp.result <- partition.MST(partition.list[[id]], MST = MST.list[[id]], graph = graph, weight = weight)
