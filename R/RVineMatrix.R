...
RVineMatrixS <- function(Matrix,
    ...
    if (sparse == TRUE) {
      MaxMat <- createMaxMat2(Matrix)
      CondDistr <- neededCondDistr2(Matrix)
...
reorderRVineMatrix2 <- function(Matrix) {
  Matrix_vector <- as.vector(Matrix)
  exchange_Entries <- function(entry, mapping) {
    ifelse(entry > 0, which(entry == mapping), 0)
  }
  out <- matrix(sapply(Matrix_vector, FUN = exchange_Entries, mapping = rev(diag(Matrix))), ncol = sqrt(length(Matrix)), nrow = sqrt(length(Matrix)))
  out
}
...
createMaxMat2 <- function(Matrix) {

    if (dim(Matrix)[1] != dim(Matrix)[2])
        stop("Structure matrix has to be quadratic.")

    MaxMat <- reorderRVineMatrix2(Matrix)
...
neededCondDistr2 <- function(Vine) {

  if (dim(Vine)[1] != dim(Vine)[2])
    stop("Structure matrix has to be quadratic.")

  Vine <- reorderRVineMatrix2(Vine)

  MaxMat <- createMaxMat2(Vine)
